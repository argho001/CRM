...
import { Download, Beaker, Loader2 } from 'lucide-react';
...
export default function TestingPage() {
  ...
  const refreshLeads = async () => {
    setLoading(true);
    const data = await getLeadsByStatus('testing');
    setLeads(data);
    setLoading(false);
  };
  ...
  const handleExport = () => {
    ...
    downloadCSV(csv, 'testing-leads.csv');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Beaker className="h-6 w-6 text-primary" />Testing Stage</h1>
          <p className="text-sm text-muted-foreground">{filtered.length} leads in quality assurance</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground border-2 border-dashed rounded-2xl">
          ...
          <p className="text-lg font-medium">No leads in Testing stage</p>
          <p className="text-sm">Move leads here from the Working section</p>
        </div>
      )}
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea value={note} onChange={e => setNote(e.target.value)} placeholder="e.g. Verified project deliverables" />
            </div>
