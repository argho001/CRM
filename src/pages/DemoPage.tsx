...
import { getLeadsByStatus, updateResponseStatus, exportToCSV, downloadCSV, updateLeadStatus } from '@/lib/lead-store';
...
export default function DemoPage() {
  ...
  const [newStatus, setNewStatus] = useState<ResponseStatus>('not_contacted');
  ...
  const refreshLeads = async () => {
    setLoading(true);
    const data = await getLeadsByStatus('demo');
    setLeads(data);
    setLoading(false);
  };
  ...
  const handleExport = () => {
    ...
    downloadCSV(csv, 'demo-leads.csv');
    toast.success('Exported to CSV');
  };

  const handleStatusChange = async (id: string, stage: string) => {
    await updateLeadStatus(id, stage as any, currentUser.id);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Presentation className="h-6 w-6 text-primary" />Demo Stage</h1>
          <p className="text-sm text-muted-foreground">{filtered.length} leads in demo verification</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground border-2 border-dashed rounded-2xl">
          ...
          <p className="text-lg font-medium">No leads in Demo stage</p>
          <p className="text-sm">Move leads here from the Done section</p>
        </div>
      )}
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea value={note} onChange={e => setNote(e.target.value)} placeholder="e.g. Discussed pricing during demo" />
            </div>
